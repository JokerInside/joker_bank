...
if Config.EnablePeds then
    AddEventHandler('esx:playerLoaded', function(playerId)
        TriggerClientEvent('joker_bank:pedHandler', playerId, netIdTable)
    end)
end

-- event
RegisterServerEvent('joker_bank:doingType')
AddEventHandler('joker_bank:doingType', function(typeData)
    ...
    if amount == nil or (not typeData.pincode and amount <= 0) then
        ...
        TriggerClientEvent("joker_bank:updateMoneyInUI", source, key, bankMoney, money)
    end
end)

-- register callbacks
ESX.RegisterServerCallback("joker_bank:getPlayerData", function(source, cb)
    local xPlayer = ESX.GetPlayerFromId(source)
    local identifier = xPlayer.getIdentifier()
...
ESX.RegisterServerCallback("joker_bank:checkPincode", function(source, cb, inputPincode)
    local xPlayer = ESX.GetPlayerFromId(source)
    local identifier = xPlayer.getIdentifier()
...
RegisterServerEvent('joker_bank:logTransaction')
AddEventHandler('joker_bank:logTransaction', function(label,key,amount)
...
BANK = {
    CreatePeds = function()
        ...
        Wait(100)
        TriggerClientEvent('joker_bank:pedHandler', -1, netIdTable)
    end,
    DeletePeds = function()
