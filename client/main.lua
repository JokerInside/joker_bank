...
--Functions
 ...
 --   Handle text ui / Keypress
    ...
    function BANK:HandleUi(state, atm)
        ...
        ESX.TriggerServerCallback('joker_bank:getPlayerData', function(data)
            SendNUIMessage({
                showMenu = true,
                openATM = atm,
                datas = {
...
-- Events
RegisterNetEvent('joker_bank:closebanking', function()
    BANK:HandleUi(false)
end)

RegisterNetEvent('joker_bank:pedHandler', function(netIdTable)
    for i = 1, #netIdTable do
        BANK:LoadNpc(i, netIdTable[i])
    end
end)

RegisterNetEvent('joker_bank:updateMoneyInUI', function(doingType, bankMoney, money)
    SendNUIMessage({
        updateData = true,
        data = {
            type = doingType,
...
RegisterNUICallback('clickButton', function(data, cb)
    ...
    TriggerServerEvent("joker_bank:doingType", data)
    cb('ok')
end)
...
RegisterNUICallback('checkPincode', function(data, cb)
    ...
    ESX.TriggerServerCallback("joker_bank:checkPincode", function(pincode)
        if pincode then
            cb({
                success = true
            })
...
RegisterNetEvent("joker:bank")
AddEventHandler("joker:bank", function()
	BANK:HandleUi(true)
	end)
RegisterNetEvent("joker:atm")
AddEventHandler("joker:atm", function()
...
Citizen.CreateThread(function ()
	for	i = 1, #Config.Banks do
		exports.ox_target:addSphereZone({
			coords 		= vec3(Config.Banks[i].Position.xyz),
			radius 		= 0.5,
			debug 		= drawZone,
			options 	= {
				{
...
local options = {{
		name 	= 'atm',
		event	= 'joker:atm',
		icon	= 'fa-solid fas fa-money-check',
		label	= 'ATM Access'
}}
